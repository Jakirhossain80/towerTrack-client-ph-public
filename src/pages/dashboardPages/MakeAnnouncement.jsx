...
const MakeAnnouncement = () => {
  ...
  return (
    <div
      className="max-w-3xl mx-auto mt-10 bg-white dark:bg-slate-100 p-6 rounded-xl shadow-md transition-all duration-500"
      data-aos="fade-up"
    >
      <h2 className="text-2xl font-semibold font-poppins text-gray-800 dark:text-gray-800 mb-6 flex items-center gap-2">
        <MdAnnouncement className="text-lime-500" /> Post Announcement
      </h2>

      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        <div>
          <label className="block text-gray-700 dark:text-gray-600 font-medium mb-1">
            Title
          </label>
          <input
            type="text"
            {...register("title", { required: "Title is required" })}
            className="w-full border border-gray-300 p-3 rounded-lg font-inter bg-gray-50 dark:bg-gray-200 text-gray-800 focus:outline-lime-500 transition-all duration-500"
          />
          {errors.title && (
