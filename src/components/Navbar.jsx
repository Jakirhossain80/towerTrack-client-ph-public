...
const Navbar = () => {
  ...
  // Prevent body scroll when mobile menu is open (helps in device toolbar)
  useEffect(() => {
    if (isMobileMenuOpen) {
  ...
  return (
    <nav
      className="fixed top-0 left-0 right-0 z-50 border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900"
      style={{ paddingTop: "env(safe-area-inset-top)" }}
      role="navigation"
      aria-label="Primary"
    >
      {/* Inner container keeps layout consistent across breakpoints */}
      <div className="max-w-screen-2xl mx-auto px-4 sm:px-6 lg:px-12">
        <div className="flex items-center justify-between py-4 sm:py-6 md:py-8">
          ...
          {/* Right: Auth, Theme, User Menu (Desktop) */}
          <div className="hidden md:flex items-center gap-6 text-gray-800 dark:text-gray-200 font-inter transition-all duration-500">
            ...
            {!user && (
              <>
                <div className="relative group">
                  ...
                  <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-2 py-1 rounded bg-gray-800 text-white text-xs opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none z-10">
                    Login
                  </div>
                </div>
            ...
            {user && (
              ...
              >
                ...
                />
                {showDropdown && (
                  <div className="absolute right-0 mt-2 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 p-3 shadow-md rounded text-sm w-44 transition-all duration-500">
                    <p className="font-medium text-gray-800 dark:text-gray-200 transition-all duration-500 truncate">
                      {user.displayName}
                    </p>
                    <NavLink
                      to="/dashboard"
      ...
      {/* Mobile NavMenu (anchored to navbar bottom) */}
      <div className="md:hidden relative">
        {isMobileMenuOpen && (
          <div
            id="mobileMenu"
            className="absolute top-full left-0 w-full bg-white dark:bg-gray-900 z-40 flex flex-col gap-4 p-5 shadow-md text-gray-800 dark:text-gray-200 font-inter transition-all duration-500 overflow-x-hidden"
          >
            {[
              ["/", "Home"],
              ["/apartment", "Apartment"],
            ...
            {!user && (
              <>
                <NavLink
                  ...
                  className="text-emerald-500 font-semibold hover:text-emerald-600 transition-all duration-500"
                >
                  <MdLogin className="w-6 h-6 inline-block align-middle mr-1" />
                  Login
                </NavLink>
            ...
            {user && (
              <div className="flex items-center gap-3">
                ...
                <div className="min-w-0">
                  <p className="font-medium transition-all duration-500 truncate">
                    {user.displayName}
                  </p>
                  <button
                    onClick={handleLogout}
